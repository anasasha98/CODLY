...
if (isset($_POST['delserv'])) {
  ...
  $serv_id = $_POST['delserv'];
  $query = "DELETE FROM `service-provider`, `purchase_list`

INNER JOIN `purchase_list`
ON `purchase_list`.`Service ID` = '$serv_id'
INNER JOIN `service-provider`
ON `service-provider`.`service_id` ='$serv_id'
 ";
