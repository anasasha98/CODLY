...
<body>
  ...
  // pressed username
  $counter = 0;
  $query = "SELECT `customerusername`, `firstname`, `lastname`, `email`, `phonenumber`, `dob`, `image` FROM `customer` where `customerusername` = '$customerusername' ";
      ...
      <main id="main" style="padding-top: 60px;">
        <div class="container-xl px-4 mt-4">
          ...
          if (isset($_POST['save'])) {
            ...
            // Update DB 
            $chnage = "UPDATE `customer` SET `firstname`='$fname',`lastname`='$lname',`phonenumber`='$phone',`dob`='$dob' WHERE `customerusername` = '$customerusername'";
          ...
          <div class="row">
            ...
            if (isset($_FILES["image"]) && !empty($_FILES["image"]["name"])) {
              if (is_uploaded_file($_FILES["image"]["tmp_name"]) && $_FILES["image"]["error"] === 0) {
                ...
                $target = "assets/img/customer_profile/" . basename($_FILES['image']['name']);

                // Get all the submitted data from the form
                $image = $_FILES['image']['name'];

                // Stores the submitted data into the database table: customer
                $sql = "UPDATE `customer` SET `image`='$image' WHERE `customerusername` = '$customerusername'";
                // Execute query
                ...
                // check before change 
                $get_image = "SELECT `image` FROM `customer` where `customerusername` = '$customerusername' ";
                $image_result = mysqli_query($con, $get_image);
                $profile_image = mysqli_fetch_assoc($image_result);

                error_reporting(E_ERROR | E_PARSE);

                $filePath = "assets/img/customer_profile/{$row['image']}";
                // echo $filePath;
                if (file_exists($filePath) && ($profile_image['image'] != $row['image'])) {
                  unlink($filePath);
            ...
            <div class="col-xl-4">
              ...
              <div class="card mb-4 mb-xl-0">
                ...
                <div class="card-body text-center">
                  ...
                  $image_result = mysqli_query($con, $get_image);
                  $profile_image = mysqli_fetch_assoc($image_result);
                  echo "<img class='img-account-profile rounded-circle mb-2' src='assets/img/customer_profile/" . $profile_image['image'] . "' alt='customer profile image'>";
                  ?>
                  <!-- Profile picture help block-->
                  <div class="small font-italic text-muted mb-4">JPG or PNG or any image format</div>

                  <!-- Profile picture upload button-->
                  <form method="POST" action="customer-account-details.php" enctype="multipart/form-data">
            ...
            <?php
            $query = "SELECT `customerusername`, `firstname`, `lastname`, `email`, `phonenumber`, `dob` FROM `customer` where `customerusername` = '$customerusername' ";
            $result = mysqli_query($con, $query);
                  ...
                  <form id="login" name="form1" method="POST" action="customer-account-details.php" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                      <label class="small mb-1" for="inputUsername">Username (how your name will appear to other users on the
                        site)</label>
                      <input class="form-control" id="inputUsername" type="text" name="customerusername" disabled placeholder="Enter your username" value="<?php echo $row['customerusername']; ?> " maxlength="25">
                    </div>
                    <!-- Form Row-->
